...
SeismicFlow includes a built-in Plugin Store for installing community-contributed algorithms and workflows. Open it from the application menu.

Plugins are hosted on GitHub — the store fetches all metadata (name, author, version, description, size) automatically from each plugin's GitHub Release. No manual configuration needed.

### Installing a Plugin

1. Open the Plugin Store from the application menu
2. Browse or search the available plugins
3. Click **Install** on any plugin
4. The plugin is downloaded and installed automatically

### Publishing a Plugin

To publish a plugin to the store:

1. Create a public GitHub repository for your plugin
2. Create a GitHub Release and attach your plugin as a `.zip` file
   - The release title becomes the plugin name
   - The release description becomes the plugin description
   - Everything else (author, version, size, date) is resolved automatically
3. Submit a Pull Request to this repository adding a single line to `plugin_registry.json`:

```json
{ "download_url": "https://github.com/YOU/YOUR_REPO/releases/download/v1.0.0/plugin.zip" }
```

That is all that is required. The store resolves all metadata live from your GitHub Release.
